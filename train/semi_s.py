...
class RGBD_Dataset(Dataset):
    def __init__(self,name,mode,path,nsample=None,size=None):
        ...
        #     img = np.array(Image.open(self.ids[id].split(' ')[0]))  
        #     depth = np.array(Image.open(self.ids[id].split(' ')[1]))
        #     output_array = np.zeros((depth.shape[0], depth.shape[1],3))
        #     output_array[ :, :,0] = depth[ :, :]  # 复制第一个通道的数据到新数组的第一个通道
        #     output_array[:, :, 1] = depth[ :, :]  # 复制第一个通道的数据到新数组的第二个通道
        #     output_array[:, :, 2] = depth[ :, :]  # 复制第一个通道的数据到新数组的第三个通道
        #     self.all_img.append(img)
        #     self.all_depth.append(output_array)
        #     if self.mode == 'train_l' or self.mode == 'val':
        #         #H,W,C
        #         if self.name == 'SUNRGBD':
        #             mask = np.load(self.ids[id].split(' ')[2],allow_pickle=True)
        #         else:
        #             mask = np.array(Image.open(self.ids[id].split(' ')[2]))
    ...
    def __getitem__(self, item):
        ...
        output_array = np.array(Image.open(id.split(' ')[1]))
        depth = np.zeros((output_array.shape[0], output_array.shape[1],3))
        depth[ :, :,0] = output_array[ :, :]  # 复制第一个通道的数据到新数组的第一个通道
        depth[:, :, 1] = output_array[ :, :]  # 复制第一个通道的数据到新数组的第二个通道
        depth[:, :, 2] = output_array[ :, :]  # 复制第一个通道的数据到新数组的第三个通道

...
def cut_mix_label_adaptive(unlabeled_image,unlabeled_depth, unlabeled_mask, unlabeled_logits, 
    ...
    # 1) get the random mixing objects  生成0 --- n-1 的整数并打乱
    ...
    for i in range(0, mix_unlabeled_image.shape[0]):
        if np.random.random() > lst_confidences[i]:
            ...
            mix_unlabeled_depth[i, :, l_bbx1[i]:l_bbx2[i], l_bby1[i]:l_bby2[i]] = \
            labeled_depth[u_rand_index[i], :, l_bbx1[i]:l_bbx2[i], l_bby1[i]:l_bby2[i]]
    ...
    del mix_unlabeled_image, mix_unlabeled_depth, mix_unlabeled_target, mix_unlabeled_logits, labeled_logits
    
    return unlabeled_image, unlabeled_depth, unlabeled_mask, unlabeled_logits 

def compute_unsupervised_loss_by_threshold(predict, target, logits, thresh=0.95):
    batch_size, num_class, h, w = predict.shape
    thresh_mask = logits.ge(thresh).bool() 

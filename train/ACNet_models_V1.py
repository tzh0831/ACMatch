...
class ACNet(nn.Module):
    ...
    def forward(self, rgb, depth, phase_checkpoint=False, need_fp=False,single=False):
        fuse0, fuse1, fuse2, fuse3, fuse4 = self.encoder(rgb, depth)
        
        if need_fp:
            ms,ms2,ms3,ms4,ms5 = self.decoder(torch.cat((fuse0, nn.Dropout2d(0.5)(fuse0))),
                             torch.cat((fuse1, nn.Dropout2d(0.5)(fuse1))),
                             torch.cat((fuse2, nn.Dropout2d(0.5)(fuse2))),
                             torch.cat((fuse3, nn.Dropout2d(0.5)(fuse3))),
                             torch.cat((fuse4, nn.Dropout2d(0.5)(fuse4))))
            ...
            m4,_ = ms4.chunk(2)
            m5,_ = ms5.chunk(2)
            return m,m_fp,m2,m3,m4,m5
        
        if  self.training:
            m,m2,m3,m4,m5 = self.decoder(fuse0, fuse1, fuse2, fuse3, fuse4)
            if single:
                return m
            else:
                return m,m2,m3,m4,m5
        else :
            m = self.decoder(fuse0, fuse1, fuse2, fuse3, fuse4)  
            return m

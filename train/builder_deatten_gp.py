...
class EncoderDecoder(nn.Module):
    ...
    def encode_decode(self, rgb, modal_x,need_fp,single):
        ...
        if need_fp:
            ms,ms2,ms3,ms4,ms5 = self.decoder.forward([torch.cat((x[0], nn.Dropout2d(0.5)(x[0]))),
                             torch.cat((x[1], nn.Dropout2d(0.5)(x[1]))),
                             torch.cat((x[2], nn.Dropout2d(0.5)(x[2]))),
                             torch.cat((x[3], nn.Dropout2d(0.5)(x[3]))),
                             torch.cat((x[4], nn.Dropout2d(0.5)(x[4])))])
            
            out ,out_fp = ms.chunk(2)
            out2,_ = ms2.chunk(2)
            out3,_ = ms3.chunk(2)
            out4,_ = ms4.chunk(2)
            out5,_ = ms5.chunk(2)
            ...
            return out,out_fp,out2,out3,out4,out5
